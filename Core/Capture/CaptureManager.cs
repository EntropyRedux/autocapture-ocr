...
/// <summary>
/// Handles screen capture operations using GDI+ / desktop capturing.
/// Implements ICaptureSource for snapshot capture mode.
/// </summary>
public class CaptureManager : ICaptureSource
{
    private bool _isRunning;
    private CaptureSourceOptions? _currentOptions;

    public string SourceName => "GDI Screen Capture";
    public CaptureSourceType SourceType => CaptureSourceType.Snapshot;
    ...
    public async Task<CapturePayload> CaptureOnceAsync(CancellationToken ct = default)
    {
        CaptureResult result;
        if (_currentOptions?.CaptureRegion != null)
        {
            var r = _currentOptions.CaptureRegion.Value;
            result = await CaptureSpecificRegionAsync(r.X, r.Y, r.Width, r.Height);
        }
        else
        {
            result = await CaptureFullscreenAsync();
        }

        if (!result.Success || result.Image == null)
        {
            throw new InvalidOperationException(result.ErrorMessage ?? "Snapshot capture failed.");
        }

        return new CapturePayload
        {
            SourceType = CaptureSourceType.Snapshot,
    ...
    {
        // Snapshot-only source does not produce continuous stream
